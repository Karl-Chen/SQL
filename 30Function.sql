...
--建立一個getProductID的函數
--其功能為新增Product時可呼叫此函數自動取得一個ProductID
--ProductID的編碼規則為「Category的ID加上3碼流水號」
--Ex: A1035為CateID為A1+第35項的同類產品，B2008為CateID為B2+第8項的同類產品
--如果即將要新增的產品CateID為A2
declare @runNum nchar(5) = dbo.getProductID('A2')
insert into [Product] values(@runNum,'test',50,'testtest',@runNum + '.jpg','A2')

Alter function getProductID(@CateID nchar(2))
	returns nchar(5)
as
begin
	--從參數取得CateID
	...
	begin
		--substring()--取字串裡面的值 例如 substring(@lastID, 2,4)
		--set @newID = substring(@lastID, 1, 1) + cast(cast(substring(@lastID, 2, 4) as int)+1 as nchar)
		declare @num int 
